...
# using CSV input files instead of an Excel workbook.
#
# This script defines:
#   1. The mathematical optimization model (build_model_mget)
#   2. How data is loaded (via prepare_mget_data_from_csv in Load_csv.py)
...
# ----------------------------------------------------------------------
# Data loader (CSV → Python dictionaries)
# ----------------------------------------------------------------------
# Load_csv.py should define:
#   DATA_DIR  – default scenario directory (e.g. ./data/scenario1)
#   prepare_mget_data_from_csv(data_dir: Path) -> dict
from Load_csv import DATA_DIR as DEFAULT_DATA_DIR, prepare_mget_data_from_csv

# Reporting utilities (printing + CSV export of results)
from report_csv import summarize_solve_status, print_text_report, export_results


# ======================================================================
# Model builder
# ======================================================================
def build_model_mget(data_dir: Path | None = None) -> pyo.ConcreteModel:
    """
    Build and return a fully constructed Pyomo model (CSV version).

    Parameters
    ----------
    data_dir : Path or None
        Path to the scenario **directory** containing the CSV files
        (nodes.csv, arcs.csv, production.csv, consumption.csv, other.csv,
        discount.csv, timeseries.csv, ...).
        If None, defaults to DEFAULT_DATA_DIR from Load_csv.py.
    ...
    # ------------------------------------------------------------------
    # prepare_mget_data_from_csv() returns a dictionary with:
    #   node_names, arc_ids, fuels, Y, H, cost parameters, efficiencies, etc.
    data = prepare_mget_data_from_csv(data_path)
    ...
    # OLD (causes KeyError if r_param / scale_param have extra keys)
    # m.r     = pyo.Param(m.Y, initialize=r_param,
    #                     within=pyo.NonNegativeReals, default=1.0)
    # m.Scale = pyo.Param(m.H, initialize=scale_param,
    #                     within=pyo.NonNegativeReals, default=1.0)

    #  use rules that only read values for indices in m.Y / m.H
    m.r = pyo.Param(
        m.Y,
        initialize=lambda mm, y: float(r_param.get(y, 1.0)),
    ...
    m.Scale = pyo.Param(
        m.H,
        initialize=lambda mm, h: float(scale_param.get(h, 1.0)),
        within=pyo.NonNegativeReals,
        default=1.0,
    )
    ...
    m.c_dz = pyo.Param(
        m.F,
        initialize=lambda mm, e: c_dz.get(e, 1e4)
    )

    # Arc transport cost and capacity
    m.c_a = pyo.Param(
        m.A, m.F, m.Y,
        initialize=lambda mm, a, e, y: c_a.get((a, e, y), 0.0)
    )
    m.cap_a = pyo.Param(
        m.A, m.F, m.Y,
        initialize=lambda mm, a, e, y: cap_a.get((a, e, y), 0.0)
    )

    # Efficiency: fraction of fuel preserved across arc conversions
    m.eff = pyo.Param(
        m.F, m.A,
        initialize=lambda mm, e, a: eff.get((e, a), 1.0)
    )

    # Investment cost for capacity expansion + binary cost
    m.c_x = pyo.Param(
        m.A, m.F, m.Y,
        initialize=lambda mm, a, e, y: c_x_dict.get((a, e, y), 0.0)
    )
    m.c_b = pyo.Param(
        m.A, m.Y,
        initialize=lambda mm, a, y: c_b_dict.get((a, y), 0.0)
    )
    ...
    m.BD    = pyo.Var(m.A, m.Y, domain=pyo.UnitInterval)                # binary decision (0–1)
...
if __name__ == "__main__":
    """
    Command-line usage:
        python mget_model_csv.py [scenario_dir] [results_root]

    Arguments
    ---------
    scenario_dir (optional)
        Path to the **directory** with CSV input files.
        If omitted, the default DATA_DIR (from Load_csv.py) is used.

    results_root (optional)
        Root folder where results will be stored.
        If omitted, results are written under '<project_root>/results/'.

    The report.py exporter will create (by default) a subfolder inside
    results_root named after the scenario directory and place
    timestamped CSV files + a runs_log.csv there.
    """
    ...
    if len(sys.argv) > 1:
        data_dir_override = Path(sys.argv[1]).expanduser().resolve()
    if len(sys.argv) > 2:
        outdir_override = Path(sys.argv[2]).expanduser().resolve()

    # Decide which scenario directory to use
    scenario_dir = data_dir_override if data_dir_override is not None else DEFAULT_DATA_DIR
    ...
    model = build_model_mget(scenario_dir)
    ...
    export_results(
        model,
        results,
        scenario_dir,          # used for naming the result folder
